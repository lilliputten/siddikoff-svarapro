...
async function getMainMenuMessage(
...
/* // UNUSED: roundToNiceNumber, generateNiceAmounts
...
export class UserHandlers {
  ...
  async handleCurrencySelection(ctx: ServiceBotContext): Promise<void> {
    ...
    if (currency === "RUB") {
      ...
      amounts.forEach((amount) => {
        buttons.push(Markup.button.callback(`${amount} ₽`, `deposit_rub_amount_${amount}`));
      });

      buttons.push(Markup.button.callback("⌨️ Ввести свою", "deposit_rub_custom_amount"));
      buttons.push(Markup.button.callback("🚫 Отмена", "cancel_operation"));
    ...
    try {
      ...
      const buttons = banks.map((bank) =>
        Markup.button.callback(bank.name, `deposit_bank_${bank.id}`),
      );
  ...
  async handleBankSelection(ctx: ServiceBotContext): Promise<void> {
    ...
    buttons.push(Markup.button.callback("⌨️ Ввести свою", "deposit_custom_amount"));
    buttons.push(Markup.button.callback("⬅️ Назад", `deposit_back_bank_${currency}`));
    buttons.push(Markup.button.callback("🚫 Отмена", "cancel_operation"));
  ...
  async handleCustomAmount(ctx: ServiceBotContext): Promise<void> {
    ...
    await ctx.editMessageText(
      `💰 Введите сумму пополнения в ${currency} (только число):\n\n` +
        ...
        (this.maxAmounts[currency]
          ? `Максимальная сумма: ${this.maxAmounts[currency].toLocaleString("ru-RU")} ${currency}\n\n`
          : `\n`) +
  ...
  async handleCustomAmountInput(ctx: ServiceBotContext, amount: number): Promise<void> {
    const userId = ctx.from?.id.toString();
    if (!userId) return;
  ...
  private async processDeposit(ctx: ServiceBotContext, userId: string): Promise<void> {
    const session = this.getSession(userId);
...
⚠️ *Важно: совершите платеж в течении 10 минут и нажмите кнопку 'Я оплатил'*`;
  ...
  async handleRateCommand(ctx: ServiceBotContext): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "Неизвестная ошибка";
      await ctx.reply(`⚠️ Ошибка при получении курсов:\n${errorMessage}\n\nПопробуйте позже.`);
    }
  }
  ...
  async handleTextMessage(ctx: ServiceBotContext): Promise<void> {
    ...
    if (depositSession.awaitingCustomAmount || depositSession.amount === -1) {
      ...
      if (isNaN(amount)) {
        await ctx.reply("⚠️ Пожалуйста, введите число. Попробуйте ещё раз или отправьте /cancel");
        return;
      }
  ...
  async handleCancelCommand(ctx: ServiceBotContext): Promise<void> {
    ...
    const mainMenuMessage = await getMainMenuMessage(
      this.apiService,
      userId,
      "🚫 Операция отменена.",
    );
  ...
  async handleWithdrawRecipientNameInput(
